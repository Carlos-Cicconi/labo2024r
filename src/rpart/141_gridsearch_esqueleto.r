...
PARAM$semillas <- c(105019, 106019, 207029, 300017, 400031)
...
for (vmax_depth in c(4, 6, 8, 10)) {
  for (vmin_split in c(800, 600, 400, 200, 100, 50)) {
    for (vcp in c(-0.1, -0.2, -0.3, 0.1)) { 
      for (vmin_bucket in c(as.integer(vmin_split/1.5), as.integer(vmin_split/1.5^2), as.integer(vmin_split/1.5^3), as.integer(vmin_split/1.5^4), as.integer(vmin_split/1.5^5), as.integer(vmin_split/1.5^6))) {
        ...
        print(list( vmax_depth, vmin_split, vcp, vmin_bucket, ganancia_promedio))
      }
    }
  }
