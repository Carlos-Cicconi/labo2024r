...
HT_tuning_epic <- function( pinputexps, bypass=FALSE)
{
  ...
  param_local$lgb_param <- list(
    ...
    extra_trees = FALSE,
    # Parte variable
    learning_rate = c( 0.09, 0.15 ), # ==> Fine tuning por experimentos anteriores
    feature_fraction = c( 0.25, 0.35 ), # ==> Fine tuning por experimentos anteriores
    
    leaf_size_log = c( -11, -6),   # ==> Fine tuning por experimentos anteriores (deriva en min_data_in_leaf)
    coverage_log = c( -5, -1 )      # ==> Fine tuning por experimentos anteriores (deriva en num_leaves)
  )
