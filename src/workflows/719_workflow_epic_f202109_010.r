...
CA_catastrophe_base <- function( pinputexps, metodo )
{
  ...
  param_local$meta$script <- "/src/wf-etapas/1202_CA_reparar_dataset_001.r"
...
FEhist_base <- function( pinputexps)
{
  ...
  param_local$lag1 <- TRUE # ==> Recomendación Experimentos Colaborativos (Astegiano)
  param_local$lag2 <- TRUE # ==> Recomendación Experimentos Colaborativos (Astegiano)
...
CN_canaritos_asesinos_base <- function( pinputexps, ratio, desvio)
{
  ...
  param_local$meta$script <- "/src/wf-etapas/1601_CN_canaritos_asesinos_001.r"
...
TS_strategy_base9 <- function( pinputexps )
{
  ...
  param_local$meta$script <- "/src/wf-etapas/2101_TS_training_strategy_001.r"
  ...
  param_local$final_train$training <- c(202104, 202103, 202102, 202101,
    202002, 202001, 201912, 201911, 201910, 201909, 201908, 201907, 201906,
    201905, 201904, 201903) #==> Según recomendación Experimentos Colaborativos


  param_local$train$training <- c(202102, 202101, 202012,
    202011, 201912, 201911, 201910, 201909, 201908, 201907, 201906, 201905, 201904,
...
HT_tuning_epic <- function( pinputexps, bypass=FALSE)
{
  ...
  param_local$lgb_param <- list(
    ...
    extra_trees = FALSE,
    # Parte variable
    learning_rate = c( 0.02, 0.03 ), # ==> Fine tuning por experimentos anteriores
    feature_fraction = c( 0.00, 0.10 ), # ==> Fine tuning por experimentos anteriores
    
    leaf_size_log = c( -9, -7),   # ==> Fine tuning por experimentos anteriores (deriva en min_data_in_leaf)
    coverage_log = c( -1, 0 )      # ==> Fine tuning por experimentos anteriores (deriva en num_leaves)
  )


  # una Bayesian razonable
  param_local$bo_iteraciones <- 10 # ==> Bajo las iteraciones porque acoté más los rangos de los hiperparámetros variables
...
{
  param_local <- exp_wf_init( pnombrewf ) # línea_fija

  DT_incorporar_dataset_competencia2024()
  
  CA_catastrophe_base( metodo="Ninguno") # ==> Según recomendación Exp Colaborativos
  
  FEintra_manual_base()
  
  DR_drifting_base(metodo="dolarb") # ==> Según recomendación Exp Colaborativos
  
  FEhist_base()
  FErf_attributes_base()
  CN_canaritos_asesinos_base(ratio=0.2, desvio=0.5) # ==> Según recomendación Exp Colaborativos

  ts9 <- TS_strategy_base9()
  ht <- HT_tuning_epic()

  fm <- FM_final_models_lightgbm_semillerio( c(ht, ts9), ranks=c(1), semillerio=50, repeticiones_exp=1 )
